...
workflow {
  ...
  input_vcf = Channel.fromPath(params.samples).
      ...
      map{row ->
        tuple(row.patient.toString(), row.timepoint.toString(), row.sample.toString(), file(row.snv_vcf_file), file(row.indel_vcf_file))} 

  input_bam = Channel.fromPath(params.samples).
    splitCsv(header: true).
    map{row ->
      tuple(row.patient.toString(), row.timepoint.toString(), row.sample.toString(), file(row.tumor_bam), file(row.normal_bam))}
